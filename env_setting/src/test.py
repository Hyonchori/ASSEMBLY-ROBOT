...
from gazebo_ros_link_attacher.srv import AttachResponse
...
def product_spawn():
	...
	part_pkg = rospack.get_path('env_setting')
	...
	for count in range(3):
		...
		for num in range(0, 5):
			x_rand = random.randrange(-20, 20) /100.0
			y_rand = random.randrange(-20, 20) /100.0
			R_rand = random.randrange(-314, 314) /100.0
			P_rand = random.randrange(-314, 314) /100.0
			Y_rand = random.randrange(-314, 314) /100.0

			quat = tf.transformations.quaternion_from_euler(R_rand, P_rand, Y_rand)
			orient = Quaternion(quat[0], quat[1], quat[2], quat[3])

			item_name = "product_{}_{}".format(count, num)
			print("Spawned model: {}".format(item_name))
...
def part_pose_collect():
	...
	for count in range(3):
		for num in range(5):
			product_num = model_pose.name.index("product_{}_{}".format(count, num))

			x = model_pose.pose[product_num].position.x
			y = model_pose.pose[product_num].position.y
			z = model_pose.pose[product_num].position.z
			rx = model_pose.pose[product_num].orientation.x
			ry = model_pose.pose[product_num].orientation.y
			rz = model_pose.pose[product_num].orientation.z
			rw = model_pose.pose[product_num].orientation.w

			euler = tf.transformations.euler_from_quaternion((rx, ry, rz, rw))
...
def gripper_control(group2, close):
...
def end_rotate(group1, yaw):

	group1.clear_pose_targets()
	
	quat = tf.transformations.quaternion_from_euler(m.radians(180), 0, yaw)
...
def robot_move_line(group1, x1, y1, z1, x2, y2, z2):

	step = 5.0

	x_step_unit = (x2 - x1)/step
	y_step_unit = (y2 - y1)/step
	z_step_unit = (z2 - z1)/step
...
def goal_move_line(group1, x, y, z):
		...
		for exec_count in range(len(model)):
			...
			if (model[exec_count][1] > -0.3):
				...
				if (model[exec_count][3] ==True):
					...
					object_degree = -m.radians(180)

				goal_move_line(group1, model[exec_count][1], model[exec_count][2], 1.3)
				end_rotate(group1, object_degree)

				robot_move_line(group1, model[exec_count][1], model[exec_count][2], 1.3, model[exec_count][1], model[exec_count][2], 0.88)

				gripper_control(group2, True)

				attach_link('robot', 'wrist_3_link', model[exec_count][0], 'link')
				rospy.sleep(1)

				robot_move_line(group1, model[exec_count][1], model[exec_count][2], 0.88, model[exec_count][1], model[exec_count][2], 1.3)

				robot_move_line(group1, model[exec_count][1], model[exec_count][2], 1.3, basket_pose[exec_count][1], basket_pose[exec_count][2], 1.3)
				rospy.sleep(0.5)

				detach_link('robot', 'wrist_3_link', model[exec_count][0], 'link')
				gripper_control(group2, False)
