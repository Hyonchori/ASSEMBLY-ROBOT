...
class UrdfCreator():
    ...
    def edit_mesh_path(self, link, target_type):
        ...
        add_path = "{}/{}/".format("//object_description", urdf_dir)
            ...
            for line in lines:
                ...
                for data in datas:
                    if "pin" in col[0]:
                        ...
                        hole_num = int(data[0]) + 1
                        hole_feature = "through" if data[12]==True else "blocked"
                        part_name = "{} {} {} {}".format(ref_name, "hole", hole_feature, hole_num)
                        print(part_name)
                        part_xyz = np.array([data[1]/1000, data[2]/1000, data[3]/1000])
                        part_zAxis = np.array([data[7], data[8], data[9]])
...
target_dir = os.path.join(r_path, "hole_pin_hj")
...
for target_name in targets:
    if "csv" not in target_name or "lock" in target_name:
        ...
        mate_urdf_path = os.path.join(r_path, urdf_dir, target_type, "urdf", urdf_name)
